...
pointToIndex p wm =
    ...
    in  if index >= 0 && index < len
            then index
                ...
                error $!
                    "Bad attempt to index a world map with region `"
                        <> (show wm.bounds)
                        <> "' at point `"
                        <> (show p)
                        <> "'"
...
renderMap f wm =
    let planeSize = fromIntegral $! wm.bounds.height * wm.bounds.width
        planeVecs =
            [ MV.slice (n * planeSize) planeSize wm.cells
  ...
  where
    ...
    renderPlane !acc !(vec, z) =
        let rowSize = fromIntegral $! wm.bounds.width
            rowVecs =
                [ MV.slice (n * rowSize) rowSize vec
        ...
        in  foldM renderRow T.empty rowVecs
                >>= \cnt ->
                    pure $!
                        acc
                            <> "<< "
                            <> (T.center (fromIntegral $! wm.bounds.width - 6) '*' $! " Plane " <> (T.pack $! show z) <> " ")
                            <> " >>\n"
                            <> cnt
                            <> "\n"
